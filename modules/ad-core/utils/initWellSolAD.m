...
if isprop(model, 'sulfate') && model.Sulfate % Scale model
	 [ws(:).qW_SO4] = deal(0);
end
if isprop(model, 'calcium') && model.calcium % Scale model
	 [ws(:).qW_Ca] = deal(0);
end
if isprop(model, 'strontium') && model.Strontium % Scale model
	 [ws(:).qW_Sr] = deal(0);
end
if isprop(model, 'barium') && model.Barium % Scale model
	 [ws(:).qW_Ba] = deal(0);
end
if isprop(model, 'sodium') && model.Sodium % Scale model
	 [ws(:).qW_Na] = deal(0);
end
if isprop(model, 'magnesium') && model.Magnesium % Scale model
	 [ws(:).qW_Mg] = deal(0);
end
if isprop(model, 'carbonate') && model.Carbonate % Scale model
	 [ws(:).qW_CO3] = deal(0);
end
if isprop(model, 'chlorine') && model.Chlorine % Scale model
	 [ws(:).qW_Cl] = deal(0);
end
...
for k = 1:nw
    ...
    if isprop(model, 'Sulfate') && model.Sulfate 
	 ws(k).c_SO4 = W(k).c_SO4*ws(k).qWs;
    end
    if isprop(model, 'Calcium') && model.Calcium 
	 ws(k).c_Ca = W(k).c_Ca*ws(k).qWs;
    end
    if isprop(model, 'Strontium') && model.Strontium
	 ws(k).c_Sr = W(k).c_Sr*ws(k).qWs;
    end
    if isprop(model, 'Barium') && model.Barium 
	 ws(k).c_Ba = W(k).c_Ba*ws(k).qWs;
    end
