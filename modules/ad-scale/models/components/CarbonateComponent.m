classdef CarbonateComponent < ConcentrationComponent
    properties

    end

    methods
        function c = CarbonateComponent()
            % sulfate transport in water phase
            wIx = 1;
            c@ConcentrationComponent('carbonate', wIx);
            
            c = c.functionDependsOn('getComponentDensity', 'carbonate',        'state');
            c = c.functionDependsOn('getComponentDensity', 'ShrinkageFactors',  'PVTPropertyFunctions');

            c = c.functionDependsOn('getComponentMass', {'s', 'carbonate'},    'state');
            ...
            c = c.functionDependsOn('getComponentMobility', 'carbonate',       'state');
        ...
        function c = getComponentDensity(component, model, state, varargin)
            c_CO3 = model.getProp(state, 'carbonate');
        ...
        function c = getInjectionMassFraction(component, model, force)
            c = vertcat(force.c_CO3)./model.fluid.rhoWS;
        end
