classdef CalciumComponent < ConcentrationComponent
    properties

    end

    methods
        function c = CalciumComponent()
            % this strontium model is transport in water phase
            wIx = 1;
            c@ConcentrationComponent('calcium', wIx);
            
            c = c.functionDependsOn('getComponentDensity', 'calcium',        'state');
            c = c.functionDependsOn('getComponentDensity', 'ShrinkageFactors',  'PVTPropertyFunctions');

            c = c.functionDependsOn('getComponentMass', {'s', 'calcium'},    'state');
            ...
            c = c.functionDependsOn('getComponentMobility', 'calcium',       'state');
        ...
        function c = getComponentDensity(component, model, state, varargin)
            c_Ca = model.getProp(state, 'calcium');
        ...
        function c = getInjectionMassFraction(component, model, force)
            c = vertcat(force.c_Ca)./model.fluid.rhoWS;
        end
