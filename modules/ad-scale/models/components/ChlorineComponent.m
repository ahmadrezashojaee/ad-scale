classdef ChlorineComponent < ConcentrationComponent
    properties

    end

    methods
        function c = ChlorineComponent()
            % sulfate transport in water phase
            wIx = 1;
            c@ConcentrationComponent('chlorine', wIx);
            
            c = c.functionDependsOn('getComponentDensity', 'chlorine',        'state');
            c = c.functionDependsOn('getComponentDensity', 'ShrinkageFactors',  'PVTPropertyFunctions');

            c = c.functionDependsOn('getComponentMass', {'s', 'chlorine'},    'state');
            ...
            c = c.functionDependsOn('getComponentMobility', 'chlorine',       'state');
        ...
        function c = getComponentDensity(component, model, state, varargin)
            c_Cl = model.getProp(state, 'chlorine');
        ...
        function c = getInjectionMassFraction(component, model, force)
            c = vertcat(force.c_Cl)./model.fluid.rhoWS;
        end
