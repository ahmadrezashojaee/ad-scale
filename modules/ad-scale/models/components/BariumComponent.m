classdef BariumComponent < ConcentrationComponent
    properties

    end

    methods
        function c = BariumComponent()
            % this strontium model is transport in water phase
            wIx = 1;
            c@ConcentrationComponent('barium', wIx);
            
            c = c.functionDependsOn('getComponentDensity', 'barium',        'state');
            c = c.functionDependsOn('getComponentDensity', 'ShrinkageFactors',  'PVTPropertyFunctions');

            c = c.functionDependsOn('getComponentMass', {'s', 'barium'},    'state');
            ...
            c = c.functionDependsOn('getComponentMobility', 'barium',       'state');
        ...
        function c = getComponentDensity(component, model, state, varargin)
            c_Ba = model.getProp(state, 'barium');
        ...
        function c = getInjectionMassFraction(component, model, force)
            c = vertcat(force.c_Ba)./model.fluid.rhoWS;
        end
